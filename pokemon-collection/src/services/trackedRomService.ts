...
export const trackedRomService = {
  ...
  async add(input: CreateTrackedRomInput): Promise<ServiceResult<TrackedRom>> {
    ...
    if (alreadyTracked) {
      return { isValid: false, errorMessages: ["That ROM is already tracked."] };
    }
  ...
  async remove(id: string): Promise<ServiceResult<{ removed: true }>> {
    if (!id.trim()) return { isValid: false, errorMessages: ["id is required."] };

    const ok = await trackedRomRepo.remove(id);
    if (!ok) return { isValid: false, errorMessages: ["Tracked ROM not found."] };
