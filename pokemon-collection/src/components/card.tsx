...
function CardImage({card}: {card: Card}) {    
    return (
        <img className="rounded-[15px] w-[400px] h-[250px]" src={card.img} alt={card.title}/>
    )
}
    ...
    return (
      <aside className="absolute top-40 left-4 bottom-4 w-[380px] flex items-end flex-wrap gap-2">
          {card.tags.map((tag, index) => (
            <a href="#" key={index} className="bg-slate-500 text-white px-[0.4rem] py-[0.1rem] rounded-[5px] tracking-[1px] hover:bg-[#475569]">{tag}</a>
          ))}
      </aside>
    )
}
